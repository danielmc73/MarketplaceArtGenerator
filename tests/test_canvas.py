...
from mag.core import ProductAssets, load_product_assets
...
def _kit_assets() -> ProductAssets:
    """Return the configured product profile for the test product."""
    product_dir = Path(__file__).resolve().parents[1] / "assets" / "products" / "kit_bola"
    return load_product_assets(product_dir)
...
class CanvasTests(unittest.TestCase):
    ...
    def test_principal_layout_is_square(self) -> None:
        product = _kit_assets()
        canvas = build_principal_image(product.ball, product.pump, pump_crop=product.pump_crop)
        self.assertEqual(canvas.render().size, (1200, 1200))

    def test_benefits_layout_is_square(self) -> None:
        product = _kit_assets()
        canvas = build_benefits_image(product.ball, product.pump, pump_crop=product.pump_crop)
        self.assertEqual(canvas.render().size, (1200, 1200))

    def test_kit_campeao_layout_is_square(self) -> None:
        product = _kit_assets()
        canvas = build_kit_campeao_image(product.ball, product.pump, pump_crop=product.pump_crop)
        self.assertEqual(canvas.render().size, (1200, 1200))

    def test_quality_layout_is_square(self) -> None:
        product = _kit_assets()
        canvas = build_quality_image(product.ball, product.pump, pump_crop=product.pump_crop)
        self.assertEqual(canvas.render().size, (1200, 1200))

    def test_questions_layout_is_square(self) -> None:
        product = _kit_assets()
        canvas = build_questions_image(product.ball, product.pump, pump_crop=product.pump_crop)
        self.assertEqual(canvas.render().size, (1200, 1200))

    def test_pump_layout_is_square(self) -> None:
        product = _kit_assets()
        canvas = build_pump_image(product.ball, product.pump, pump_crop=product.pump_crop)
        self.assertEqual(canvas.render().size, (1200, 1200))

    def test_versatility_layout_is_square(self) -> None:
        product = _kit_assets()
        canvas = build_versatility_image(product.ball, product.pump, pump_crop=product.pump_crop)
        self.assertEqual(canvas.render().size, (1200, 1200))

    def test_gift_layout_is_square(self) -> None:
        product = _kit_assets()
        canvas = build_gift_image(product.ball, product.pump, pump_crop=product.pump_crop)
        self.assertEqual(canvas.render().size, (1200, 1200))
